...
for (res in resolutions) {
  ...
  p <- ggplot(umap_df, aes(x = UMAP_1, y = UMAP_2, color = cluster)) +
    ...
    label_layer +
    ggtitle(paste0("UMAP – Résolution = ", res, " (", n_clusters, " clusters)")) +
    theme_minimal() +
    theme(
...
Neural$cluster <- Neural$RNA_snn_res.0.4  # pour l’analyse à résolution 0.4
...
p_D <- base_plot +
  ...
  ggtitle("UMAP – cellules D (contrôle) en noir")
p_D
...
p_N <- base_plot +
  ...
  ggtitle("UMAP – cellules N (nicotine) en noir")
p_N
...
barplot_cluster_cond <- ggplot(umap_df, aes(x = factor(Neural$RNA_snn_res.0.4), fill = condition)) +
  ...
  scale_fill_manual(values = cols_cond) +
  labs(title = "Proportion de chaque condition par cluster (résolution 0.4)",
       x = "Cluster",
       y = "Proportion") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
```{r}
# Import des gènes de cycle (Seurat possède déjà la liste humaine)
cc.genes <- Seurat::cc.genes.updated.2019


# Vérification des gènes présents dans ton dataset
...
## DEGs
```{r}
# Définir les identités sur la condition
...
# Extraire l'expression de ces gènes
expr_matrix <- GetAssayData(Neural, assay = "RNA", slot = "data")[DEG_genes, ]


# Nombre de DEGs exprimés (>0) par cellule
...
## Top 10 gènes les plus exprimés par cluster
```{r}
...
# Vérification : affichage rapide des clusters détectés
unique(all_markers$cluster)


# Sélection du top 10 gènes les plus spécifiques par cluster
top10 <- all_markers %>%
  group_by(cluster) %>%
  slice_max(order_by = avg_log2FC, n = 10) %>%
  ungroup() %>%
  mutate(cluster = as.numeric(as.character(cluster)))  # conversion en numérique pour tri correct
...
## FeaturePlot des gènes clés du papier de référence (Guo et al., 2019, Single-Cell RNA Sequencing of Human Embryonic Stem Cell Differentiation Delineates Adverse Effects of Nicotine on Embryonic Development - doi: 10.1016/j.stemcr.2019.01.022)
```{r}
genes_paper <- c(
  ###### CHATGPT
  # Pluripotence
  ...
  # Mésoderme
  "MESP1",   # Non-exprimé dans Neural : QC
...
"DDIT3", # stress response (ER stress) - déjà dans la liste
...
# Vérifie quels gènes sont présents dans ton objet
genes_paper_present <- genes_paper[genes_paper %in% rownames(Neural)]
cat("Nombre de gènes trouvés dans le dataset :", length(genes_paper_present), "\n")
...
```{r}
# Fixe l’identité active sur la condition
Idents(Neural) <- "Condition"
...
DEG_N_vs_D <- FindMarkers(
  ...
  only.pos = FALSE,
  assay = "RNA",
  logfc.threshold = 0.2,  # paramètre moins restricitfs
  min.pct = 0.05
)
...
# Sélection des 50 gènes les plus différentiels
...
## Croiser les DEG et les genes les plus exprimé par cluster
Quels gènes, caractéristiques d’un sous-cluster neuronal, sont aussi modulés par la condition N vs D ?
```{r}

Idents(Neural) <- "RNA_snn_res.0.4"

# On va prendre, par exemple, les top gènes par cluster
...
# Puis garder les top N gènes par cluster (ex. 20)
top_markers_clusters <- markers_clusters %>%
  group_by(cluster) %>%
  slice_max(order_by = avg_log2FC, n = 20) %>%
  ungroup()

# Croisement DEG ∩ gènes fortement exprimés
...
############# HEATMAPS #############
# Montre où ces gènes sont exprimés dans l’espace cellulaire
...
DoHeatmap(
  ...
  ggtitle("DEG N vs D ∩ Cluster markers – par cluster")
dev.off()


# Effet condition N vs D (moyenné)
...
DoHeatmap(
  ...
  ggtitle("DEG N vs D ∩ Cluster markers – par condition")
dev.off()
```
...
eye_development <- c("LH++X2", "IGFBP5")
...
# Pour relier à ta GSEA nicotine.
FeaturePlot(
  Neural,
...
barplot_cluster_cond <- ggplot(umap_df, aes(x = factor(Neural$RNA_snn_res.0.7), fill = condition)) +
  ...
  labs(title = "Proportion de chaque condition par cluster (résolution 0.7)",
       x = "Cluster",
       y = "Proportion") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
trajectory_order <- c(
  ...
  "Neural progenitors, sensory neuron-like" = "Sensory neuron–like"
)
...
```
Nicotine exposure does not primarily enhance proliferation but biases neural progenitor fate toward early sensory neuron and eye-field–related lineages, while reducing pluripotent-like states.


### Cluster de départ pour la trajectoire : Neuroepithelial stem cells
```{r}
...
# Extraction des coordonnées UMAP pour les barcodes sélectionnés
...
**Distribution bimodale** parce que les 2 conditions montrent 2 pics de densité, (~1.5 et ~13-14). Suggère 2 états cellulaires principaux le long de la trajectoire :
Premier pic → cellules plus “immatures” ou précoces
Deuxième pic → cellules plus “matures” ou tardives

Différence entre conditions :
D a légèrement plus de densité dans le pic initial (~0-2) → plus de cellules immatures.
N a un pic plus marqué vers le pseudotime tardif (~12-14) → plus de cellules différenciées.

## Interprétation SOX11 (FT) le long de la trajectoire
stem → progenitors → differentiating (SOX11 intervient ici) → immature neuron
exprimé dans neurones immatures / différenciation neuronale précoce / disparaît dans les neurones matures
Classique dans neurogenèse / cortex (forebrain) / neurones sensoriels en développement
Ici :
cluster 7 : SOX11 UP ++
cluster 3 : cycle cellulaire (SOX11 down)
cluster 6 : neuroépithélial (SOX11 down)
SOX11 augmente avec le pseudotime -> gène lié à la maturation et potentiellement initié par la nicotine.
...
# Trouver les DEGs entre N et D  (fait précédemment mais avec des paramètres différents)
res <- FindMarkers(
  Neural,
  ident.1 = "N",  # Nicotine
  ident.2 = "D",  # Contrôle
...
## Hallmark plots
```{r}
gsea_df <- read.csv("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/csv_files_results/neurons/GSEA_Hallmarks_N_vs_D.csv")
...
p_gsea <- ggplot(
  ...
  labs(
    x = "NES",
    y = NULL,
    title = "GSEA – Top 12 pathways (N vs D)"
  ) +
...
p_gsea
ggsave("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/plot/neurons/GSEA_Top12_Hallmarks_N_vs_D.png", plot = p_gsea, width = 7, height = 3, dpi = 300)
...
p_gseapos <- ggplot(
  ...
  labs(
    x = "NES",
    y = NULL,
    title = "GSEA – Top 12 pathways positifs (N vs D)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    legend.position = "none",
...
p_gseapos
ggsave("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/plot/neurons/GSEA_Top12_Hallmarks_Positive_N_vs_D.png", plot = p_gseapos, width = 7, height = 3, dpi = 300)
```

## Interpretation
* L’axe **NES > 0 (rouge)** correspond à des pathways **enrichis dans la condition N**
* L’axe **NES < 0 (beige)** correspond à des pathways **enrichis dans la condition D**
* On regarde donc **quels programmes biologiques sont activés ou réprimés par la nicotine (N) par rapport au contrôle (D)**


UP dans N
**Activation forte des programmes prolifératifs / biosynthétiques**
Les 2 signaux les plus forts sont :
* **HALLMARK_MYC_TARGETS_V1**
* **HALLMARK_E2F_TARGETS**
-> augmentation de :
  * transcription globale
  * biogenèse ribosomale
  * cycle cellulaire / entrée en phase S
* Chez les neurones, cela **ne veut pas dire prolifération**, mais plutôt :
  * **stress transcriptionnel**
  * **augmentation de la demande métabolique**
  * parfois un **état immature / réactif**
-> signal classique d’**hyperactivité cellulaire**.

**Métabolisme mitochondrial et stress protéique**
...
Les neurones sous nicotine :
  ...
  * accumulent des protéines mal repliées
  * activent des mécanismes de réparation
Profil compatible avec :
* **sur-activation neuronale chronique**
* **stress oxydatif secondaire**

**ROS et métabolisme lipidique**
* **REACTIVE_OXYGEN_SPECIES_PATHWAY**
* **ADIPOGENESIS**
* Activation des défenses antioxydantes (PRDX, SOD, TXN…)
* Ajustements métaboliques (lipides, membranes)
Très cohérent avec :
* activation mitochondriale
* dérégulation redox induite par la nicotine


DOWN dans N / UP dans D
**Réponse hypoxique**
* **HALLMARK_HYPOXIA**
* Les neurones D sont plus proches d’un état hypoxique / glycolytique
* Dans N, au contraire :
  * préférence pour l’**OXPHOS**
  * baisse de la signature HIF-1α
Transition **glycolyse → mitochondries** sous nicotine.
...
* Sous nicotine, * ces voies sont **réprimées dans les neurones**


**La nicotine induit chez les neurones un état d’hyperactivité métabolique et transcriptionnelle (MYC/E2F, OXPHOS, UPR, ROS), au prix d’un stress cellulaire accru, tandis que les neurones contrôle présentent un profil plus inflammatoire et hypoxique.**

Cohérence avec l’article de référence
Totalement cohérent avec :
...
> la nicotine ne “calme” pas les neurones, elle les **pousse dans un état énergétiquement coûteux**.

> *L’analyse GSEA a révélé un fort enrichissement des programmes transcriptionnels dirigés par MYC et E2F, la phosphorylation oxydative, la réponse protéique dépliée et les voies ROS dans les neurones exposés à la nicotine, indiquant une activité métabolique et transcriptionnelle accrue. En revanche, l’hypoxie et les voies de signalisation inflammatoires (TNF alpha/NFκ B, IL6/STAT3) étaient relativement enrichies dans les neurones témoins.*

--- 

## Hypoxic control
**les gènes d’hypoxie sont plus exprimés dans D (contrôle)** que dans N.
Ce n’est pas “il y a de l’hypoxie”, c’est :
> *les gènes associés à une réponse hypoxique sont relativement plus actifs dans D que dans N.*

 🔹 Hypothèse 1 — **DOWN Nicotine signature hypoxique**
La nicotine est connue pour :
* activer **mTOR / MYC / OXPHOS**
* augmenter le métabolisme mitochondrial
* favoriser un état **plus “énergétique”**

Résultat logique :
* N = métabolisme + OXPHOS UP
* D = état basal → **signature hypoxie relative**

* OXPHOS UP dans N
* Hypoxia DOWN dans N (donc UP relatif dans D)
 **C’est cohérent.**

 🔹 Hypothèse 2 — Hypoxia ≠ manque d’oxygène réel
...
Présence de gènes :
* de **quiescence**
* de **métabolisme “économe”**
* de stress adaptatif
-> Le contrôle peut être **plus quiescent**, pas “hypoxique”.

🔹 Hypothèse 3 — Nicotine = état pro-prolifératif
Dans  plot :
**UP dans N**
* MYC targets
* E2F targets
...
Ça décrit très bien :
* **N** : état prolifératif / métaboliquement actif
* **D** : état plus “stress basal / inflammatoire”

Est-ce que UP dans D par rapport à N ?
```{r}
...
```
Ce que dit le papier de référence 

* La nicotine **modifie profondément le métabolisme neuronal**
* Elle :
  * UP respiration mitochondriale
  * UP consommation d’oxygène
  * DOWN signatures de stress hypoxique
* Effet paradoxal :
  > nicotine peut **réduire l’activation HIF1α** malgré des conditions normales
Donc **voir Hypoxia enrichie dans le témoin est parfaitement compatible avec le papier**.




## ROS sont plus exprimés dans N que dans D ? (papier dit oui)
```{r}
# Ne garder que les neurones
neurons <- subset(
  Neural,
  subset = annotated_cluster %in% c(
    "Differentiating neurons",
    "Neural progenitors, sensory neuron-like"
  )
)
...
genes_up <- rownames(deg_sig[deg_sig$avg_log2FC > 0, ])
...
ros_terms <- ego_df[
  grepl(
    "oxidative|hypoxia|reactive oxygen|unfolded",
    ego_df$Description,
    ignore.case = TRUE
  ),
]
...
ros_terms <- unique(unlist(strsplit(ros_terms$geneID, "/")))

# Module score ROS
neurons <- AddModuleScore(
  neurons,
  features = list(ros_terms),
  name = "ROS_score"
)
#head(neurons@meta.data$ROS_score1)

Neural$ROS_score1 <- NA
Neural$ROS_score1[colnames(neurons)] <- neurons$ROS_score1
summary(Neural$ROS_score1)

#saveRDS(Neural,"C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/data/rds_files/Neural_progenitor_cells.rds")

# Dataframe pour stats + plot
df_ros <- data.frame(
  ROS = neurons$ROS_score1,
  Condition = neurons$Condition
)

# Wilcoxon test (N > D)
wilcox_ros <- wilcox.test(
  ROS ~ Condition,
  data = df_ros,
  alternative = "greater"
)
...
# Plot
y_max <- max(df_ros$ROS, na.rm = TRUE) * 0.9

ggplot(df_ros, aes(x = Condition, y = ROS, fill = Condition)) +
  stat_summary(fun = mean, geom = "bar", width = 0.6, color = "black") +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2) +
  stat_compare_means(
    method = "wilcox.test",
    alternative = "greater",
    comparisons = list(c("D", "N")),
    label = "p.signif"
  ) +
  scale_fill_manual(values = c("N" = N, "D" = D)) +

  theme_classic(base_size = 16) +
  ylab("Score de stress oxydatif (A.U., centré)") +
  xlab("") +
  ggtitle("Activation du stress oxydatif")

ggsave("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/plot/neurons/ROS_score_neurons_N_vs_D.png",dpi = 300, width = 5, height = 5)
...
ggplot(df_ros, aes(x = Condition, y = ROS, fill = Condition)) +
  ...
  ggtitle("Activation du stress oxydatif") +
  NoLegend()
ggsave("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/plot/neurons/ROS_score_neurons_N_vs_D_noleg.png",dpi = 300, width = 4, height = 4)

#Le score de stress oxydatif correspond à un score d’expression relative (module score) centré autour de zéro. Ainsi, la comparaison entre conditions est interprétée de manière relative, indépendamment de la valeur absolue du score.
```

## Plot
```{r}
library(viridis)
head(Neural$ROS_score1)
...
FeaturePlot(
  ...
  ggtitle("Score de stress oxydatif (ROS) – Neurones") +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold")
  )
...
# version ggplot
df_umap <- as.data.frame(Embeddings(Neural, "umap"))
df_umap$ROS <- Neural$ROS_score1
df_umap$Condition <- Neural$Condition

ggplot(df_umap, aes(UMAP_1, UMAP_2, color = ROS)) +
  geom_point(size = 0.5) +
  scale_color_viridis(option = "viridis") +
  theme_void(base_size = 14) +
  ggtitle("Projection du score ROS sur l’UMAP (neurones)") +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold")
  )
...
FeaturePlot(
  Neural,
  features = c("BNIP3", "MT1X", "HSP90AA1"),
  reduction = "umap",
  cols = viridis(256),
  ncol = 3
)
...
# Retirer les NA (sécurité)
df_ros <- df_ros[!is.na(df_ros$ROS), ]

neurons$Condition <- factor(neurons$Condition, levels = c("N", "D"))
# Wilcoxon test (N > D)
wilcox_ros <- wilcox.test(
  ROS ~ Condition,
  data = df_ros,
...
p_vln_ros <- ggplot(df_ros, aes(x = Condition, y = ROS, fill = Condition)) +
  ...
  # Test de Wilcoxon directement sur le plot
  stat_compare_means(
    method = "wilcox.test",
    alternative = "greater",
    comparisons = list(c("N", "D")),
    label = "p.signif",
    size = 6
  ) +
  ...
  labs(
    title = "Score de stress oxydatif dans les neurones",
    subtitle = "Comparaison Nicotine (N) vs Contrôle (D)",
    y = "Score de stress oxydatif (A.U.)",
    x = ""
  ) +
...
p_vln_ros
ggsave("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/plot/neurons/ROS_score_neurons_N_vs_D_violin.png", plot = p_vln_ros, dpi = 300, width = 5, height = 5)

```
La nicotine induit un stress oxydatif dans une sous-population de neurones, sans augmentation globale homogène du ROS dans l’ensemble des neurones.

# GOTERM ANALYSIS
```{r}
go_table <- as.data.frame(ego)

# Construire colonne Genes avec tous les gènes du GO term séparés par "_"
go_table$Genes <- sapply(as.character(go_table$geneID), function(g) {
  genes <- unlist(strsplit(g, "[^A-Za-z0-9_/]+"))  # séparer par tout séparateur non alphanumérique
  paste(genes, collapse = "_")
})

# Calculer Score et Zscore
go_table <- go_table %>%
  mutate(
    Pathway  = paste0(Description, ":", ID),
    Score    = -log10(pvalue) * Count,
    Zscore   = as.numeric(scale(Count))  # Zscore sur Count original
  )

# Construire le tableau final
go_final <- go_table %>%
  select(Pathway, Genes, Pvalue = pvalue, nGenes = Count, Score, Zscore, FDR = p.adjust)

# Optionnel : filtrer uniquement les pathways avec au moins un gène dans le dataset Seurat
# si tu veux absolument ne garder que ceux présents dans ton objet Neural :
# go_final <- go_final %>% filter(sapply(strsplit(Genes, "_"), function(x) any(x %in% rownames(Neural))))

write.table(go_final,file = "C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/csv_files_results/neurons/GO_enrichment_neurons_nicotine.csv",sep = ",",row.names = FALSE,col.names = TRUE,quote = FALSE,fileEncoding = "UTF-8")
```
L’exposition à la nicotine induit une activation accrue des voies liées au stress oxydatif, à la réponse au stress cellulaire et au métabolisme énergétique dans les neurones, suggérant une reprogrammation métabolique associée à une accélération de la différenciation neuronale.


## Plot go term 
```{r}
# Charger le tableau
go_final <- read.csv("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/csv_files_results/neurons/GO_enrichment_neurons_nicotine.csv", sep = ";", header = TRUE)
colnames(go_final)


# Sélectionner les top 12 par Score absolu (tu peux aussi utiliser -log10(FDR) si tu préfères)
top_go <- go_final %>%
...
xlim <- max(abs(top_go$Score))
xmax <- max(top_go$Score, na.rm = TRUE) * 1.05
...
p_go <- ggplot(
  ...
  labs(
    x = "Score d'enrichissement",
    y = NULL,
    title = "Top 12 GO Terms – Neurones (N vs D)"
  ) +
...
ggsave("C:/Users/tomga/Desktop/github/Nicotine_Impact_on_Neurogenesis_snRNAseq/plot/neurons/GO_Enrichment_Top12.png", plot = p_go, width = 7, height = 3, dpi = 300)
```
